...
def test_CorruptedCodeQLDataException_message():
    ...
    assert str(
        exception) == "CodeQL data in file some/invalid/path behaves unexpectedly: there should be file"
...
def test_map_directories_to_names_corrupted_data(capsys):
    ...
    assert captured_message.out == "CodeQL data in file tests/test_query_data/one-col-table.csv behaves unexpectedly: there should be exactly 2 columns in this table, but there is 1\n"
    assert res == ({}, {})

def test_map_directories_to_names_file_not_found(capsys):
    res = map_directories_to_names(f"invalid_path")
    captured_message = capsys.readouterr()
...
def test_map_directories_to_names_wrong_file_type(capsys):
    ...
    captured_message = capsys.readouterr()
    assert captured_message.out == "Exception while reading file tests/test_query_data/not_csv.png: 'utf-8' codec can't decode byte 0x89 in position 0: invalid start byte\n"
    assert res == ({}, {})
...
def test_map_frontend_rest_requests_corrupted_data(capsys):
    ...
    captured_message = capsys.readouterr()
    assert captured_message.out == "CodeQL data in file tests/test_query_data/one-col-table.csv behaves unexpectedly: there should be exactly 5 columns in this table, but there is 1\n"
    assert res == {}

def test_map_frontend_rest_requests_file_not_found(capsys):
    res = map_frontend_rest_requests(f"invalid_path")
    captured_message = capsys.readouterr()
    assert captured_message.out == "File not found: invalid_path\n"
    assert res == {}
...
def test_map_frontend_rest_requests_valid_file():
    res = map_frontend_rest_requests(f"{TEST_TABLES_DIR}/routes-of-services-in-frontend.csv")
    expected =   {
       'customers-service': (
...
def test_map_services_to_names_file_not_found(capsys):
    res = map_services_to_names(f"invalid_path", {})
    captured_message = capsys.readouterr()
    assert captured_message.out == "File not found: invalid_path\n"
    assert res == {}
...
def test_map_backend_rest_requests_corrupted_data(capsys):
    ...
    captured_message = capsys.readouterr()
    assert captured_message.out == "CodeQL data in file tests/test_query_data/one-col-table.csv behaves unexpectedly: there should be exactly 3 columns in this table, but there is 1\n"
    assert res == set()

def test_map_backend_rest_requests_file_not_found(capsys):
    res = map_backend_rest_requests(f"invalid_path", {})
    captured_message = capsys.readouterr()
    assert captured_message.out == "File not found: invalid_path\n"
    assert res == set()
...
def test_map_backend_rest_requests_valid_file():
    ...
    res = map_backend_rest_requests(f"{TEST_TABLES_DIR}/rest-requesters.csv", dir_to_name)
    assert res == expected

def test_extract_config_server_file_not_found(capsys):
    res = extract_config_server(f"invalid_path")
    captured_message = capsys.readouterr()
    assert captured_message.out == "File not found: invalid_path\n"
    assert res == set()
...
def test_map_config_server_to_link_file_not_found(capsys):
    res = map_config_server_to_link(f"invalid_path", set())
    captured_message = capsys.readouterr()
    assert captured_message.out == "File not found: invalid_path\n"
    assert res == {}
...
def test_map_config_server_to_link_valid_file():
    ...
    expected = {'spring-petclinic-config-server': 'https://github.com/spring-petclinic/spring-petclinic-microservices-config'}
    res = map_config_server_to_link(f"{TEST_TABLES_DIR}/config-server-link.csv", servers)
    assert res == expected
